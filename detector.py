...
def arg_parse():
    ...
    # cfg is the alterative configuration file
    parser.add_argument("--cfg", dest='cfgfile', help="Config file",
                        default="cfg/yolov3.cfg", type=str)
    # weight file
    parser.add_argument("--weights", dest='weightsfile', help="weightsfile",
                        default="yolov3.weights", type=str)
    # reso is the input image's resolution, can be used for speed-accuracy tradeoff
...
# inialize the network and load weights
args = arg_parse()
images = args.images
...
def letterbox_imgae(img, inp_dim):
    """
    resize image with unchanged sspect ratio using padding
    """
...
for i, batch in enumerate(im_batches):
    ...
    if CUDA:
        # this line makes sure that CUDA is synchronized with the CPU.
...
# his is a pickled file that contains many colors to randomly choose from.
...
def write(x, results, color):
    ...
    cv2.rectangle(img, (int(c1[0]), int(c1[1])),
                  (int(c2[0]), int(c2[1])), color, 1)
...
list(map(lambda x: write(x, loaded_ims,
     colors[random.randint(0, len(colors)-1)]), output))
